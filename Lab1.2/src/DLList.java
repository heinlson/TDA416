...
public class DLList {
	...
	*/
	public int readShape() throws FileNotFoundException {
		Scanner sc = new Scanner(new File("D:\\Users\\ridda\\OneDrive\\Projects\\TDA416\\Lab1.2\\src\\fig4.txt"));
		int n = 0;
	...
	*/
	public void reduceListToKElements(int k) {
        ...
        while (size > k) {
            ...
            size--;
            updateNeighbours(leastImp);
        }
	}

	private void updateNeighbours(Node n) {
	    if (n.prev != head) {
            n.prev.imp = importanceOfP(n.prev.prev.p, n.prev.p, n.prev.next.p);
        }
        if (n.next != tail) {
            n.next.imp = importanceOfP(n.next.prev.p, n.next.p, n.next.next.p);
        }
		q.remove(n.prev);
	    q.remove(n.next);
	    q.add(n.prev);
